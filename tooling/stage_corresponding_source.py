...
PYTETWILD_BUILD_RECIPE_PATH = "tooling/recipes/BUILD_PYTETWILD_WINDOWS_20260823.ps1"
...
SOURCE_EXECUTABLE_PATHS_NAME = "SOURCE_EXECUTABLE_PATHS.txt"
MAX_ARCHIVE_MEMBERS = 500_000
MAX_ARCHIVE_UNCOMPRESSED_BYTES = 40 * 1024 * 1024 * 1024
MAX_GIT_SYMLINK_TARGET_BYTES = 4096
MAX_GIT_UNIQUE_SYMLINK_BLOBS = 10_000
MAX_GIT_SYMLINK_RESOLUTION_DEPTH = 64
MAX_GIT_SYMLINK_RESOLUTION_STEPS = 100_000
MAX_GIT_SYMLINK_MATERIALIZED_ENTRIES = 100_000
...
GIT_LOCAL_ENVIRONMENT_PREFIXES = ("GIT_CONFIG_KEY_", "GIT_CONFIG_VALUE_")
...
def _git_environment() -> dict[str, str]:
    ...
    for name in tuple(environment):
        ...
        if normalized in GIT_LOCAL_ENVIRONMENT_VARIABLES or normalized.startswith(
            GIT_LOCAL_ENVIRONMENT_PREFIXES
        ):
...
def _verify_exact_pinned_git_commitish(
    repository: Path,
    pinned_object_id: str,
    *,
    allow_file: bool = False,
) -> str:
    """Verify an exact commit or annotated-tag object and return its peeled commit."""

    if not isinstance(pinned_object_id, str) or not SHA1_RE.fullmatch(
        pinned_object_id
    ):
        raise StageError("Pinned Git object must be a full lowercase SHA-1")
    exact_object = _run_git(
        repository,
        ["rev-parse", "--verify", f"{pinned_object_id}^{{object}}"],
        allow_file=allow_file,
    ).strip()
    if exact_object != pinned_object_id:
        raise StageError("Git did not resolve the exact pinned object")
    object_type = _run_git(
        repository,
        ["cat-file", "-t", pinned_object_id],
        allow_file=allow_file,
    ).strip()
    if object_type not in {"commit", "tag"}:
        raise StageError("Pinned Git object must be a commit or annotated tag")
    peeled_commit = _run_git(
        repository,
        ["rev-parse", "--verify", f"{pinned_object_id}^{{commit}}"],
        allow_file=allow_file,
    ).strip()
    if not SHA1_RE.fullmatch(peeled_commit):
        raise StageError("Pinned Git object did not peel to one full commit")
    if object_type == "commit" and peeled_commit != pinned_object_id:
        raise StageError("Pinned Git commit did not preserve its exact object identity")
    ...
    *,
    ...
    if repository.exists():
        ...
        try:
            ...
            try:
                _verify_exact_pinned_git_commitish(
                    temporary,
                    commit,
                    allow_file=True,
                )
                ...
                raise StageError(
                    f"Git commit verification failed for {component_id}: {exc}"
                ) from exc
    ...
    try:
        ...
        raise StageError(
            f"Cached commit verification failed for {component_id}: {exc}"
        ) from exc
    return repository
    ...
    with tarfile.open(archive, mode="r:*") as package:
        for member in package:
            ...
            if member.mode & 0o111:
                executable_paths.add("/".join(parts))
...
def _read_git_symlink_blob(
    ...
    size_result = subprocess.run(
        ["git", "-C", str(repository), "cat-file", "-s", object_id],
        check=False,
        capture_output=True,
        env=environment,
    )
    if size_result.returncode != 0:
        ...
        raise StageError(
            f"Cannot read Git symlink target size for {relative}: "
            f"{detail or size_result.returncode}"
        )
    try:
        size = int(size_result.stdout.decode("ascii", errors="strict").strip())
    except (UnicodeError, ValueError) as exc:
        raise StageError(f"Git symlink target size is invalid: {relative}") from exc
    if size < 1 or size > MAX_GIT_SYMLINK_TARGET_BYTES:
        raise StageError(f"Git symlink target size is invalid: {relative}")
    ...
    if blob_result.returncode != 0:
        ...
        raise StageError(
            f"Cannot read Git symlink target for {relative}: "
            f"{detail or blob_result.returncode}"
        )
    if len(blob_result.stdout) != size:
        raise StageError(f"Git symlink target was truncated: {relative}")
    return blob_result.stdout


def _decode_git_symlink_target(relative: str, payload: bytes) -> tuple[str, ...]:
    if b"\x00" in payload:
        raise StageError(f"Git symlink target contains NUL: {relative}")
    try:
        target = payload.decode("utf-8", errors="strict")
    except UnicodeDecodeError as exc:
        raise StageError(f"Git symlink target is not UTF-8: {relative}") from exc
    if any(unicodedata.category(character) == "Cc" for character in target):
        raise StageError(f"Git symlink target contains a control character: {relative}")
    if (
        not target
        or "\\" in target
        ...
        raise StageError(f"Git symlink target is not relative: {relative}")

    components = tuple(target.split("/"))
    for part in components:
        if part in ("", ".", ".."):
            continue
        _safe_relative(part, f"Git symlink target component from {relative}")
    return components


def _export_git_tree(repository: Path, commit: str, destination: Path) -> set[str]:
    if destination.exists():
    ...
    seen: set[str] = set()
    unique_symlink_blobs: set[str] = set()
    for raw_entry in tree_result.stdout.split(b"\x00"):
        ...
        if mode == "120000":
            if object_type != "blob":
                raise StageError(f"Git symlink is not a blob: {relative}")
            unique_symlink_blobs.add(object_id)
            if len(unique_symlink_blobs) > MAX_GIT_UNIQUE_SYMLINK_BLOBS:
                raise StageError("Git source exceeds the unique symlink blob limit")
            entries.append((mode, object_type, object_id, relative))
            continue
    ...
    entries_by_path = {entry[3]: entry for entry in entries}
    children_by_tree: dict[str, list[str]] = {}
    for _mode, _object_type, _object_id, relative in entries:
        parent = PurePosixPath(relative).parent.as_posix()
        if parent == ".":
            parent = ""
        children_by_tree.setdefault(parent, []).append(relative)
    for children in children_by_tree.values():
        children.sort(key=lambda path: (path.casefold(), path))

    symlink_targets: dict[str, tuple[str, ...]] = {}
    symlink_blob_targets: dict[str, tuple[str, ...]] = {}
    output_nodes: dict[str, tuple[str, str]] = {}
    output_files: dict[str, tuple[str, str]] = {}
    materialized_nodes: set[str] = set()
    resolution_steps = 0

    def register_node(relative: str, kind: str, *, materialized: bool) -> None:
        relative = _safe_relative(relative, "materialized Git output path")
        folded = relative.casefold()
        existing = output_nodes.get(folded)
        if existing is not None:
            existing_path, existing_kind = existing
            if existing_path != relative:
                raise StageError(
                    f"Materialized Git output has a casefold collision: {relative}"
                )
            if existing_kind != kind:
                raise StageError(
                    f"Materialized Git output has a file-ancestor collision: {relative}"
                )
            if kind == "file":
                raise StageError(f"Materialized Git output path collides: {relative}")
        else:
            output_nodes[folded] = (relative, kind)
        if materialized:
            materialized_nodes.add(folded)
            if len(materialized_nodes) > MAX_GIT_SYMLINK_MATERIALIZED_ENTRIES:
                raise StageError("Git symlink materialization exceeds the entry limit")

    def register_directory(relative: str, *, materialized: bool) -> None:
    ...
    def register_file(
        ...
        *,
        ...
        parent = PurePosixPath(relative).parent.as_posix()
        if parent != ".":
            register_directory(parent, materialized=materialized)
        register_node(relative, "file", materialized=materialized)
        output_files[relative] = (mode, object_id)

    def symlink_target(relative: str, object_id: str) -> tuple[str, ...]:
        components = symlink_targets.get(relative)
        if components is None:
            components = symlink_blob_targets.get(object_id)
            if components is None:
                payload = _read_git_symlink_blob(
                    repository,
                    object_id,
                    relative,
                    environment,
                )
                components = _decode_git_symlink_target(relative, payload)
                symlink_blob_targets[object_id] = components
            symlink_targets[relative] = components
        return components

    def is_tree(source_path: str) -> bool:
        if not source_path:
            return True
    ...
    def resolve_components(
        ...
        if depth > MAX_GIT_SYMLINK_RESOLUTION_DEPTH:
            raise StageError("Git symlink materialization exceeds the depth limit")
        current = base_path
        followed: set[str] = set()
        for part in components:
            resolution_steps += 1
            if resolution_steps > MAX_GIT_SYMLINK_RESOLUTION_STEPS:
                raise StageError(
                    "Git symlink materialization exceeds the resolution step limit"
                )
            if not is_tree(current):
                raise StageError(
                    f"Git symlink traversal reached a non-tree entry: {current}"
                )
            if part in ("", "."):
                continue
            if part == "..":
                if not current:
                    raise StageError("Git symlink target escapes the tree")
                parent = PurePosixPath(current).parent.as_posix()
                current = "" if parent == "." else parent
                continue

            candidate = f"{current}/{part}" if current else part
            candidate = _safe_relative(candidate, "resolved Git symlink target")
            entry = entries_by_path.get(candidate)
            if entry is None:
                raise StageError(f"Git symlink target is missing: {candidate}")
            mode, object_type, _object_id, _relative = entry
            if mode == "120000" and object_type == "blob":
                current, nested_followed = resolve_symlink_entry(
                    candidate,
                    active_symlinks,
                    depth + 1,
                )
                followed.update(nested_followed)
            else:
                current = candidate
        return current, frozenset(followed)
    ...
    def resolve_symlink_entry(
        ...
        if source_path in active_symlinks:
            raise StageError(f"Git symlink cycle detected from {source_path}")
        entry = entries_by_path.get(source_path)
        if entry is None:
            raise StageError(f"Git symlink target is missing: {source_path}")
        mode, object_type, object_id, _relative = entry
        if mode != "120000" or object_type != "blob":
            raise StageError(f"Git symlink target is not a symlink: {source_path}")
        parent = PurePosixPath(source_path).parent.as_posix()
        base_path = "" if parent == "." else parent
        target, nested_followed = resolve_components(
            base_path,
            symlink_target(source_path, object_id),
            active_symlinks | {source_path},
            depth,
        )
        return target, nested_followed | {source_path}
    ...
    def expand_entry(
        ...
        *,
        ...
        if depth > MAX_GIT_SYMLINK_RESOLUTION_DEPTH:
            raise StageError("Git symlink materialization exceeds the depth limit")
        if not source_path:
            mode, object_type, object_id, _entry_path = (
                "040000",
                "tree",
                "",
                "",
            ...
            if entry is None:
                raise StageError(f"Git symlink target is missing: {source_path}")
            mode, object_type, object_id, _entry_path = entry
        if mode in ("100644", "100755") and object_type == "blob":
            ...
            return
        if mode == "040000" and object_type == "tree":
            register_directory(output_path, materialized=materialized)
            for child in children_by_tree.get(source_path, []):
            ...
            return
        if mode == "120000" and object_type == "blob":
            target, followed_symlinks = resolve_symlink_entry(
                source_path,
                active_symlinks,
                depth + 1,
            )
            expand_entry(
                target,
                output_path,
                active_symlinks | followed_symlinks,
                depth + 1,
                materialized=True,
            )
            return
        if mode == "160000" and object_type == "commit":
            if materialized:
                raise StageError(
                    f"Git symlink materialization reached a gitlink: {source_path}"
                )
            register_directory(output_path, materialized=False)
            return
        raise StageError(
            f"Git symlink target has an unsupported mode or type: {source_path}"
        )

    for mode, object_type, _object_id, relative in sorted(
        entries,
        key=lambda entry: (entry[3].casefold(), entry[3]),
    ):
        if mode == "040000" and object_type == "tree":
            continue
        expand_entry(
    ...
    directories = sorted(
        (
            path
            for path, kind in output_nodes.values()
            if kind == "directory"
        ),
        key=lambda path: (len(PurePosixPath(path).parts), path.casefold(), path),
    )
    for relative in directories:
        destination.joinpath(*PurePosixPath(relative).parts).mkdir(exist_ok=False)
    ...
    try:
        ...
        for relative in sorted(output_files, key=lambda path: (path.casefold(), path)):
            mode, object_id = output_files[relative]
            ...
            if (
                len(header_fields) != 3
                or header_fields[0] != object_id.encode("ascii")
                or header_fields[1] != b"blob"
            ):
            ...
            if remaining < 0:
                raise StageError(f"git cat-file returned an invalid size for {relative}")
            total_output_file_bytes += remaining
            if total_output_file_bytes > MAX_ARCHIVE_UNCOMPRESSED_BYTES:
                raise StageError("Git export exceeds the total output file byte limit")
    ...
    return {
        relative
        for relative, (mode, _object_id) in output_files.items()
        if mode == "100755"
    }
...
def _pytetwild_layout_input_contract(recipe_sha256: str) -> dict:
    # Select from the already byte-bound recipe, never from an attestation's
    # self-declared layout. The immutable approved wheel predates the later
    # developer recipe and was built with the original 714-file layout.
    if recipe_sha256 == (
    ...
    return {
        "visual_studio_layout_sha256": "f8da7bbaed18d9e26dbf5eeee0e0fad01758656727c5afa959844c623c23943a",
        "visual_studio_installer_opc_sha256": "854ca62cdd88af1f9c57ce176fde8db1ba62c97b64325038896cc2fd2cbd135e",
        "visual_studio_installer_root_certificate_sha256": "df545bf919a2439c36983b54cdfc903dfa4f37d3996d8d84b4c31eec6f3c163e",
        "visual_studio_layout_file_count": 677,
        "visual_studio_layout_total_bytes": 2651027432,
        "visual_studio_layout_tree_sha256": "c3e3ee7cb01e2a2e73be7e9fba4b7c298ef9687bbaed302965e3270c38d37bba",
    }
...
def _verify_pytetwild_attestation_binding(
    ...
    expected_inputs = {
        ...
        **_pytetwild_layout_input_contract(bound_evidence["build_recipe"]["sha256"]),
...
class _Stager:
    def __init__(
        ...
        *,
        ...
        self._executable_paths_by_fold: dict[str, str] = {}
    ...
    def _record_executable_paths(
        self,
        destination: Path,
        relative_paths: set[str],
    ) -> None:
        for relative in relative_paths:
            relative = _safe_relative(relative, "staged executable path")
            target = destination.joinpath(*PurePosixPath(relative).parts)
            if not target.is_file() or target.is_symlink():
                raise StageError(f"Staged executable is not a regular file: {relative}")
            staged_relative = target.relative_to(self.stage_root).as_posix()
            staged_relative = _safe_relative(
                staged_relative,
                "bundle executable path",
            )
            folded = staged_relative.casefold()
            previous = self._executable_paths_by_fold.get(folded)
            if previous is not None and previous != staged_relative:
                raise StageError(
                    f"Case-insensitive staged executable collision: {staged_relative}"
                )
            self._executable_paths_by_fold[folded] = staged_relative
            self.executable_paths.add(staged_relative)
    ...
    def _stage_git_component(
        ...
        *,
        ...
        executable_paths = _export_git_tree(repository, component["commit"], destination)
        self._record_executable_paths(destination, executable_paths)
    ...
    def _stage_archive_component(self, component: dict) -> None:
        ...
        if component["stage_mode"] == "preserve":
            ...
            executable_paths = (
                _safe_extract_zip(cached, destination)
                if zipfile.is_zipfile(cached)
                else _safe_extract_tar(cached, destination)
            )
            self._record_executable_paths(destination, executable_paths)
        record = {
            "destination": component["destination"],
    ...
    def _stage_project(self) -> None:
        ...
        destination.parent.mkdir(parents=True, exist_ok=True)
        executable_paths = _export_git_tree(
            self.project_repository,
            self.project_commit,
            destination,
        )
...
def _parse_executable_paths(payload: str) -> set[str]:
    if "\r" in payload or (payload and not payload.endswith("\n")):
        raise StageError("Executable-path metadata has invalid line endings")
    lines = payload[:-1].split("\n") if payload else []
    if any(not line for line in lines):
        raise StageError("Executable-path metadata contains an empty path")
    canonical = sorted(lines, key=lambda path: (path.casefold(), path))
    if lines != canonical:
        raise StageError("Executable-path metadata is not canonically ordered")

    result: set[str] = set()
    folded_paths: set[str] = set()
    for relative in lines:
        relative = _safe_relative(relative, "executable-path metadata entry")
        if relative.casefold() in {
            SOURCE_EXECUTABLE_PATHS_NAME.casefold(),
            "SOURCE_MANIFEST_SHA256.txt".casefold(),
        }:
            raise StageError(f"Executable-path metadata lists a reserved file: {relative}")
        folded = relative.casefold()
        if folded in folded_paths:
            raise StageError(
                f"Executable-path metadata has a case-insensitive duplicate: {relative}"
            )
        folded_paths.add(folded)
        result.add(relative)
    return result
...
def _write_executable_paths(root: Path, executable_paths: set[str]) -> Path:
    metadata_path = root / SOURCE_EXECUTABLE_PATHS_NAME
    if metadata_path.exists():
        raise StageError(f"Staged source already contains {SOURCE_EXECUTABLE_PATHS_NAME}")
    canonical = sorted(executable_paths, key=lambda path: (path.casefold(), path))
    parsed = _parse_executable_paths(
        ("\n".join(canonical) + "\n") if canonical else ""
    )
    if parsed != set(canonical):
        raise StageError("Executable-path metadata changed during canonicalization")
    for relative in canonical:
        target = root.joinpath(*PurePosixPath(relative).parts)
        if not target.is_file() or target.is_symlink():
            raise StageError(f"Executable-path metadata target is not a file: {relative}")
...
def _read_executable_paths(root: Path) -> set[str]:
    metadata_path = root / SOURCE_EXECUTABLE_PATHS_NAME
    if not metadata_path.is_file() or metadata_path.is_symlink():
        raise StageError(f"Staged source lacks {SOURCE_EXECUTABLE_PATHS_NAME}")
    try:
        payload = metadata_path.read_text(encoding="utf-8", errors="strict")
    except UnicodeDecodeError as exc:
        raise StageError("Executable-path metadata is not UTF-8") from exc
    executable_paths = _parse_executable_paths(payload)
    for relative in executable_paths:
        target = root.joinpath(*PurePosixPath(relative).parts)
        if not target.is_file() or target.is_symlink():
            raise StageError(f"Executable-path metadata target is unknown: {relative}")
    return executable_paths
...
def _create_deterministic_zip(root: Path, archive: Path, archive_root_name: str) -> None:
    ...
    executable_paths = _read_executable_paths(root)
...
def _verify_output_zip(archive: Path, root_name: str) -> None:
    ...
    with zipfile.ZipFile(archive) as package:
        ...
        known_ancestors: set[tuple[str, ...]] = set()
        for item in package.infolist():
            raw_name, parts, member_kind, mode = _zip_member_details(item)
            if parts is None or not raw_name.startswith(prefix):
            ...
            if not _register_archive_member(
                ...
                raise StageError(
                    f"Output archive has a duplicate directory: {raw_name!r}"
                )

            if item.create_system != 3:
                raise StageError(
                    f"Output archive has an unsupported origin platform: {raw_name!r}"
                )
            expected_type = stat.S_IFDIR if member_kind == "directory" else stat.S_IFREG
            permissions = stat.S_IMODE(mode)
            allowed_permissions = {0o755} if member_kind == "directory" else {0o644, 0o755}
            if stat.S_IFMT(mode) != expected_type or permissions not in allowed_permissions:
                raise StageError(
                    f"Output archive has a non-canonical Unix mode: {raw_name!r}"
                )
            if member_kind == "file":
                names.append(raw_name)
                file_modes[raw_name] = mode
        ...
        executable_metadata_name = f"{prefix}{SOURCE_EXECUTABLE_PATHS_NAME}"
        if executable_metadata_name not in names:
            raise StageError(f"Output archive lacks {SOURCE_EXECUTABLE_PATHS_NAME}")
        ...
        try:
            executable_payload = package.read(executable_metadata_name).decode(
                "utf-8",
                errors="strict",
            )
        except UnicodeDecodeError as exc:
            raise StageError("Output executable-path metadata is not UTF-8") from exc
        executable_paths = _parse_executable_paths(executable_payload)
        archive_relative_names = {name[len(prefix) :] for name in names}
        unknown_executables = sorted(executable_paths - archive_relative_names)
        if unknown_executables:
            raise StageError(
                f"Output executable-path metadata has unknown paths: {unknown_executables}"
            )
        for name, mode in file_modes.items():
            ...
            if mode != expected_mode:
                raise StageError(f"Output archive mode disagrees with metadata: {name}")
...
def stage_bundle(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    try:
        ...
        _write_executable_paths(temporary_stage, stager.executable_paths)
