...
公開済み`v0.8beta-r32`はimmutableなprevious evidenceで、tag、asset、checksumを差し替えません。r32.1はWindows ZIP、完全対応ソース、SBOM、component map、privacy確認済み操作動画、`SHA256SUMS-r32.1.txt`を一組としてだけ同時公開します。r32.1のexact regression、build、package、archive gateは現在pendingです。

r32.1 Windows package候補には、ownerが公開・再配布を承認したHi3D分割GLBとreference画像を`DemoData/`へ同梱します。固定size、SHA-256、注意事項、権利gateは記録済みで、final package bytesの照合はpendingです。GLBは **Generated by Hi3D** であり、Hi3D対応は非公式βです。単独のEXE／ZIPは正式なr32.1配布物ではありません。
