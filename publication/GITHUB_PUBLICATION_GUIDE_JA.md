...
- `README.md`: GitHubのトップに自動表示する英語README
- `README_EN.md`: `README.md`とbyte-identicalな英語alias
- `README_JA.md`: 日本語版
- `LICENSE`: GitHubが認識しやすいルートのGPL version 3全文

`README.md`、`README_EN.md`、`README_JA.md`、`LICENSE`が公開候補のルートに存在することを確認します。`licenses/LICENSE_APP.txt`と`licenses/THIRD_PARTY_LICENSES.txt`も残します。`LICENSE`だけにまとめて第三者ライセンスを削除してはいけません。
