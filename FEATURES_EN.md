...
> **Windows prerelease:** [Download ChromaMatter 0.8beta r32.1 directly](https://github.com/Ponkichi0718/ChromaMatter/releases/download/v0.8beta-r32.1/ChromaMatter-0.8beta-r32.1-win64.zip). Extract the whole ZIP before launching it. See the [r32.1 prerelease page](https://github.com/Ponkichi0718/ChromaMatter/releases/tag/v0.8beta-r32.1) for checksums, source, and the other matching assets.

> **r32.2 candidate:** the default-branch source is preparing a new package
> with one combined and six part-specific derived demo 3MF projects. It is not
> published yet; all exact build, source, archive, and checksum gates remain
> pending, so r32.1 above remains the current immutable download.
...
- `v0.8beta-r32.1` is now available as a **prerelease** for evaluation: [direct Windows ZIP download](https://github.com/Ponkichi0718/ChromaMatter/releases/download/v0.8beta-r32.1/ChromaMatter-0.8beta-r32.1-win64.zip). Use the matching source, compliance assets, and checksums on the same prerelease page rather than an independently copied executable.
- r32.2 is a local release candidate. Its planned `DemoData/3MF/` contains one combined project and six part-specific projects derived from the published demo GLB. Hi3D-derived part labels can disagree with visible geometry, so inspect each project rather than trusting its filename. When regenerating the demo, always apply the **Weak Black 5–25%** preset before export.
