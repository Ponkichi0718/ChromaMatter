...
- r32.1 will be published only as a simultaneous asset set: Windows ZIP, complete corresponding source, SBOM, component map, privacy-reviewed workflow video, and `SHA256SUMS-r32.1.txt`. Its exact regression, build, package, and archive gates are still pending.
- The r32.1 Windows package candidate includes an owner-approved Hi3D multipart GLB and reference image under `DemoData/`. Their fixed sizes, hashes, notices, and rights gate are recorded; final package-byte verification remains pending. The GLB is **Generated by Hi3D**, and support is unofficial beta compatibility.
- A standalone executable or ZIP is not an authoritative r32.1 distribution; the matching complete corresponding-source asset and checksum record are mandatory.
