...
class PublicManifestAuditTests(unittest.TestCase):
    ...
    def test_configured_private_audit_token_is_trimmed_and_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "public-stage"
            root.mkdir()
            private_identifier = "-".join(
                ("synthetic", "private", "audit", "identifier")
            )
            (root / "leak.txt").write_text(
                f"operator value: {private_identifier}\n",
                encoding="utf-8",
            )
            environment = {
                **os.environ,
                "CHROMAMATTER_PRIVATE_AUDIT_TOKENS": (
                    f"  {private_identifier} ; {private_identifier.upper()}  "
                ),
            }
            ...
            output = result.stdout + result.stderr
            self.assertNotEqual(result.returncode, 0, output)
            self.assertIn("forbidden configured private text token", output)
            self.assertNotIn(private_identifier, output.casefold())

    def test_configured_private_audit_token_in_path_is_redacted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "public-stage"
            root.mkdir()
            private_identifier = "-".join(
                ("synthetic", "private", "path", "identifier")
            )
            (root / f"{private_identifier}.txt").write_text(
                "ordinary public-looking content\n",
                encoding="utf-8",
            )
            _write_source_manifest(root)
            environment = {
                **os.environ,
                "CHROMAMATTER_PRIVATE_AUDIT_TOKENS": private_identifier,
            }
            ...
            output = result.stdout + result.stderr
            self.assertNotEqual(result.returncode, 0, output)
            self.assertIn("forbidden configured private token in path", output)
            self.assertIn("<redacted-path>", output)
            self.assertNotIn(private_identifier, output.casefold())

    def test_configured_private_audit_token_empty_segment_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "public-stage"
            root.mkdir()
            (root / "ordinary.txt").write_text("public data\n", encoding="utf-8")
            environment = {
                **os.environ,
                "CHROMAMATTER_PRIVATE_AUDIT_TOKENS": "synthetic-private-token;",
            }
            ...
            output = result.stdout + result.stderr
            self.assertNotEqual(result.returncode, 0, output)
            self.assertIn("contains an empty token", output)
...
class SoftwarePackageStageTests(unittest.TestCase):
    ...
    def test_sensitive_text_inside_internal_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            built = self._fake_build(root)
            private_path = "C:" + "\\Users\\synthetic-user\\model"
    ...
    def test_sensitive_identifier_inside_binary_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            built = self._fake_build(root)
            private_identifier_text = "-".join(
                ("synthetic", "private", "binary", "identifier")
            )
            private_identifier = private_identifier_text.encode("ascii")
            ...
            environment = {
                **os.environ,
                "CHROMAMATTER_PRIVATE_AUDIT_TOKENS": private_identifier_text,
            }
            result = _run_powershell(
                SOFTWARE_STAGE_SCRIPT,
                "-BuiltAppRoot",
            ...
            output = result.stdout + result.stderr
            self.assertNotEqual(result.returncode, 0, output)
            self.assertIn("forbidden configured private binary token", output)
            self.assertNotIn(private_identifier_text, output.casefold())
            self.assertFalse(destination.exists())
            self.assertFalse(Path(f"{destination}.zip").exists())
            self._assert_no_stage_debris(root, destination)

    def test_sensitive_identifier_in_payload_path_is_redacted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            built = self._fake_build(root)
            private_identifier = "-".join(
                ("synthetic", "private", "payload", "path")
            )
            (built / "_internal" / f"{private_identifier}.txt").write_text(
                "ordinary public-looking content\n",
                encoding="utf-8",
            )
            destination = root / "rejected-private-payload-path"
            environment = {
                **os.environ,
                "CHROMAMATTER_PRIVATE_AUDIT_TOKENS": private_identifier,
            }
...
class PublicSourceStageRollbackTests(unittest.TestCase):
    ...
    def test_privacy_audit_failure_rolls_back_only_new_stage(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            fixture = self._source_fixture(root)
            private_identifier = "-".join(
                ("synthetic", "private", "source", "identifier")
            )
