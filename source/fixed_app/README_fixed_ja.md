# ChromaMatter — AI Model Print Studio 0.8beta（r32.2 source候補）
...
表示versionは`0.8beta`、現在のローカル候補editionは
...
r32.2はr32.1のapplicationとfail-closed 3MF契約を維持し、`DemoData/3MF/`へ
結合6 mesh project 1件とpart別project 6件、計7件の派生demo出力を追加する
候補です。Hi3D由来labelは見た目のgeometryと一致しないためfilenameで判断せず、
demoを再出力するときは3MF出力前に**必ず「黒弱め 5〜25%」presetを適用**します。
パーツ化modelの閉立体化はまだ不安定なbeta互換です。r32.2のexact build、source、
package、fresh extract、公開gateはpendingです。

## r32.1 公開済みprevious evidence

r32.1はr32のmodel処理／3MF契約を維持し、公開preview表記を「AIモデル色」へ
変更し、権利確認済みのHi3D分割GLBと元画像を`DemoData/`へ同梱しました。
prereleaseはimmutableな公開済みprevious evidenceで、r32.2を検証しません。
...
- 変更中worktreeのfocused testは92/92 PASS、full regressionは1,295 PASS／3 optional SKIP／0 FAILで、manifest固定済みDemoDataのpreflightもPASSしています。
- exact commitのclean build、packaged／fresh-extracted self-test、日英UI smoke、source／software stage、最終archive／privacy／checksum検証、公開: **pending**。
- source publication eligibility: **false**。binary publication eligibility: exact r32.2 gate完了まで**false**。
- 以下のr32.1結果はimmutableなprevious evidenceだけです。
...
- r32.1 source publication eligibility: **true**。r32.1 binary publication eligibility: 上記exact tagged assetだけに対して**true**。
- 以下のr32.1／r32／r31結果は**previous evidence**であり、r32.2を検証しない
...
- 現行r32.2のsource publication eligibility: false。exact stageはpending
- 現行r32.2のbinary publication eligibility: false。exact build／package gateはpending
...
公開済みr32.1／r32 Releaseと上記r31、r30、r29、r28、r27の結果は各revision
だけの**previous evidence**で、r32.2を検証しません。2026-08-20のcreator
declarationとproject ownerによる公開GOは、ロボットと創作文言
`ZENITH DYNAMICS CORP.`を含む変更していないicon assetについて保持します。
これは独立した商標／意匠clearanceや法的意見ではありません。分割modelの
閉立体化は開示済みbeta制約として残ります。

Creator Studio r26の結果も同様にprevious evidenceで、ChromaMatter r31へ流用しません。

## Package identity

- 予定candidate完全対応source: `ChromaMatter-0.8beta-r32.2-complete-corresponding-source.zip`
- 予定candidate Windows package: `ChromaMatter-0.8beta-r32.2-win64.zip`
