# ChromaMatter — AI Model Print Studio 0.8beta (r31) provenance

更新日: 2026-08-20  
対象revision: `r31-ai-model-print-studio`
...
表示versionは利用者の指定どおり`0.8beta`、Windows数値versionは`0.8.0.0`に固定し、人向けeditionを`AI Model Print Studio r31`とします。

- public source: `ChromaMatter_0.8beta-r31-ai-model-print-studio_HANDOFF`
- software package: `ChromaMatter_0.8beta-r31-ai-model-print-studio`

## 2. r31 identity更新とr30機能継承
...
Creator Studio r30には次のsoftware evidenceがあります。ChromaMatterの公開名・アイコン・実行file identityを変更した現在のr31 exact sourceには流用しない**previous evidence**です。
...
この証拠はr27 artifactだけに適用し、r31 source、binary、package、checksumの検証には流用しません。preflight ZIP hashはcanonical文書へ埋め込みません。

r30以前のregression／build／artifact evidenceはr31 sourceへ流用しません。project ownerは2026-08-20に公開GOを出し、icon publication rightsはcreator declarationによりpass、当該asset scopeのlegal gateはowner acceptanceとして完了しました。これは独立した法務clearanceではありません。最終source-only stage `ChromaMatter_0.8beta-r31-source-public-20260820`は227 files／226 manifest recordsで、folder／archive parity、CRC、privacy、staged identity／icon／tooling 32 testsを全て通過しました。Downloads配置と外部detached `SHA256SUMS-r31.txt`の照合もsource-only distributionについて完了しています。source-only repositoryはpublic owner handle `Ponkichi0718`により[https://github.com/Ponkichi0718/ChromaMatter](https://github.com/Ponkichi0718/ChromaMatter)で公開され、個人メールを含まない初回公開commitは`a01ba4baa791809a5a6621fac35956dc65479216`です。checksumの正本は外部detached recordだけとし、canonical文書のZIP hash欄は自己参照回避のため`null`のままです。release stateは`source-published`、`publication_eligible=true`のscopeは`source-only`に限定します。EXE／software ZIPの`binary_publication_eligible`は第三者binary再配布監査が終わるまでfalseです。Innovation Fund提出は権利処理済みsample、Orca／U1 evidence、cover／video／community postが揃うまでreadyではありません。physical XP-PEN validationとphysical printer testは既知の未完了evidence／制約として開示しますが、source公開を差し戻すhard gateにはしません。

## 5. Previous evidence

Creator Studio r30、r29、r28、r27の上記結果は**previous evidence only**であり、ChromaMatter r31のsource、binary、package、checksumを検証しません。
...
配布checksumはfinal restage後にDownloadsへ作成した外部`SHA256SUMS-r31.txt`だけを正本とします。canonical文書へfinal ZIPの自己参照hashは埋め込みません。
